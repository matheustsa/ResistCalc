...
class DICResultado : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for ((index, T) in lista2.withIndex()) {
            media.add(Utils.roundTo(T.average(), 4)!!)
        ...
        val mediaGlobal = Utils.roundTo((lista2.sumByDouble { it.sum() }/(k*n)), 4)
        println("=================== MEDIA GLOBAL:" + lista2.sumByDouble { it.sum() })

        val QM_Dentro = Utils.roundTo(variancia.sum() / k, 4)!!
//       N * (m0 - m_global)^2 + (m1 - m_global)^2 + ...
        ...
        QM_Entre *= n
        QM_Entre = Utils.roundTo(QM_Entre, 4)!!

        val F_Calculado = Utils.roundTo(QM_Entre / QM_Dentro, 4)
        val F_Critico = Utils.roundTo(FDistribution(gl1.toDouble(), gl2.toDouble()).inverseCumulativeProbability(1.0 - alfa), 4)
